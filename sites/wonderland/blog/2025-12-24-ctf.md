...
The script is left as an exercise to the reader, because I can't find the one I wrote.
...
This is a somewhat obscure tidbit of knowledge that some CTF players may have. I wanted to reward the nerdiness. But either way, it's not quick to execute, even less if you don't have the scripts. So, to be honest, this was slightly out of place for a short CTF with multiple challenges.

The main idea is explained in this tweet: 

![Faketoshi signature tweet](/img/blog-posts-img/ctf/faketoshi-signature-tweet.png)

This tweet comes [from an article](https://jimmysong.medium.com/faketoshis-nonsense-signature-8700a44536b5) that explains the whole process. I recommend reading it. The gist is that given enough degrees of freedom, like a signer choosing a non-hashed message, then we can create valid signatures that recover to a target `signer`. The restriction is that we can't control the resulting `message` itself. As in, it will be mostly gibberish.
...
To get the scripts to solve this,  you can visit [this gist](https://gist.github.com/0xng/f2783f9bd5db2d1df3111475a6446937) I compiled which uses a script from [this article](https://kebabsec.xyz/posts/rareskills-alphagoatclub-ctf/) by @shanduquar and a python one for the public key done by GPT because I couldn't fine the one I had written for another CTF.
And yes, I'm aware my organization of files is terrible. 

Like I mentioned, with this approach you could create more than the needed number of valid signatures to approve and execute all proposed transactions.
...
Note: the explanation for this one may be slightly inaccurate as I had this written in a `README` and can't recall if I did modifications. However, the main vulnerability is properly explained. Will double check it once I return to work.
...
I have annotated the snippet and will do my best to explain it, although this one is slightly challenging to explain in words - it's way easier to see it visually, but there are some contextual things that can help with understanding what's happening:
